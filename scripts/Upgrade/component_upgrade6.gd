...
func _on_game_start_setup_ui() -> void:
	...
	if not upgrade6:
		upgrade6 = Upgrade06AttackMinCritChance.new()
	...
	upgrade6.leveled_up.connect(update_label_title)
	upgrade6.leveled_up.connect(update_description)
	upgrade6.leveled_up.connect(update_button)
	

func update_label_title() -> void:
	var text : String = upgrade6.title + " (%s)" %upgrade6.level
	label_title.text = text

func update_description() -> void:
	label_description.text = upgrade6.description()

func update_button(_quantity : int = -1) -> void:
	if upgrade6.can_afford():
