...
func _on_game_start_setup_ui() -> void:
	...
	if not upgrade3:
		upgrade3 = Upgrade03AttackCriticalDamage.new()
	...
	upgrade3.leveled_up.connect(update_label_title)
	upgrade3.leveled_up.connect(update_description)
	upgrade3.leveled_up.connect(update_button)
	

func update_label_title() -> void:
	var text : String = upgrade3.title + " (%s)" %upgrade3.level
	label_title.text = text

func update_description() -> void:
	label_description.text = upgrade3.description()

func update_button(_quantity : int = -1) -> void:
	if upgrade3.can_afford():
